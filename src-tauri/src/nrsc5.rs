    ...
    impl Nrsc5State {
        ...
        pub fn stop_thread(&self, window: Window) {
            if let Ok(mut nrsc5) = self.0.try_lock() {
                ...
                window
                    .emit("nrsc5_status", Some("stopped"))
        ...
        fn handle_nrsc5_output(window: &Window, line: String) {
            if line.starts_with("Found") {
                window
                    .emit("nrsc5_status", Some("sdr-found"))
                    .expect("failed to emit event");
                ...
                if (!message.starts_with("Lost synchronization")) {
                    ...
                    window
                        .emit("nrsc5_status", Some("synchronization_lost"))
                        .expect("failed to emit event");
                }

                if message.starts_with("Title: ") {
                    window
                        .emit("nrsc5_title", message.strip_prefix("Title: "))
                        .expect("failed to emit event");
                } else if message.starts_with("Artist: ") {
                    window
                        .emit("nrsc5_artist", message.strip_prefix("Artist: "))
                        .expect("failed to emit event");
                } else if message.starts_with("Audio bit rate: ") {
                    window
                        .emit("nrsc5_br", message.strip_prefix("Audio bit rate: "))
                        .expect("failed to emit event");
                } else if message.starts_with("Station name: ") {
                    window
                        .emit("nrsc5_station", message.strip_prefix("Station name: "))
                        .expect("failed to emit event");
                } else if message.starts_with("Slogan: ") {
                    window
                        .emit("nrsc5_slogan", message.strip_prefix("Slogan: "))
                        .expect("failed to emit event");
                } else if message.starts_with("Message: ") {
                    window
                        .emit("nrsc5_message", message.strip_prefix("Message: "))
                        .expect("failed to emit event");
